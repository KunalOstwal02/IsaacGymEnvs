...
# [Kunal] This is the next step after removing props.
#         In this, the reward function will be changed to only reach not open cabinet.
class FrankaCabinetOnlyReach(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        max_agg_bodies = num_franka_bodies + num_cabinet_bodies
        max_agg_shapes = num_franka_shapes + num_cabinet_shapes
    ...
    def reset_idx(self, env_ids):
        ...
        # print("reset franka cabinet envs: ", env_ids)
...
def compute_franka_reward(
    ...
    # print(f'First distance: {d[0]}')
    dist_reward = 1.0 / (1.0 + d)
    # [Kunal] - Simplifying the reward function to just focus on minimizing the distance
    # dist_reward *= dist_reward
    # dist_reward = torch.where(d <= 0.02, dist_reward * 2, dist_reward)

    rewards = dist_reward_scale * dist_reward

    # [Kunal] - Removing as the orientation of the hand is not important & focus is on minimizing the distance
    ...
    ## [Kunal] Removing the cabinet opening parts
    ...
    ## [Kunal] - Removing as condition to reset is not dependent on cabinet position
    # reset if drawer is open or max length reached
    # reset_buf = torch.where(cabinet_dof_pos[:, 3] > 0.39, torch.ones_like(reset_buf), reset_buf)  ## Reset
