...
class FrankaCabinetChangeTarget(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        box_asset_file = "urdf/sektion_cabinet_model/urdf/box1.urdf"
        ...
        for i in range(self.num_envs):
            ...
            box_actor = self.gym.create_actor(env_ptr, box_asset, box_pose, "box", i, 3, 0)
            self.gym.set_actor_dof_properties(env_ptr, box_actor, box_dof_props)

            # [Kunal] - Create cylinder actor
            cylinder_pose = box_start_pose
    ...
    def init_data(self):
        ...
        franka_local_grasp_pose.p += gymapi.Vec3(*get_axis_params(0.000001, grasp_pose_axis))
        ...
        self.drawer_local_grasp_pos = to_torch([cylinder_local_grasp_pose.p.x, cylinder_local_grasp_pose.p.y,
                                                cylinder_local_grasp_pose.p.z], device=self.device).repeat(
            (self.num_envs, 1))
        self.drawer_local_grasp_rot = to_torch([cylinder_local_grasp_pose.r.x, cylinder_local_grasp_pose.r.y,
        ...
        # [Kunal] - Cylinder axes
        self.cylinder_inward_axis = to_torch([1, 0, 0], device=self.device).repeat((self.num_envs, 1))
        self.cylinder_up_axis = to_torch([0, 0, 0], device=self.device).repeat((self.num_envs, 1))
        ...
        self.cylinder_grasp_pos = torch.zeros_like(self.drawer_local_grasp_pos)
        self.cylinder_grasp_rot = torch.zeros_like(self.drawer_local_grasp_rot)
    ...
    def compute_observations(self):
        ...
        self.franka_grasp_rot[:], self.franka_grasp_pos[:], self.cylinder_grasp_rot[:], self.cylinder_grasp_pos[:] = \
            compute_grasp_transforms(hand_rot, hand_pos, self.franka_local_grasp_rot, self.franka_local_grasp_pos,
                                     cylinder_rot, cylinder_pos, self.drawer_local_grasp_rot, self.drawer_local_grasp_pos
                                     )
...
def compute_franka_reward(
    ...
    d = torch.norm(franka_grasp_pos - cylinder_grasp_pos, p=2, dim=-1)
    ...
    around_handle_reward = torch.where(franka_lfinger_pos[:, 1] > cylinder_grasp_pos[:, 1],
                                       torch.where(franka_rfinger_pos[:, 1] < cylinder_grasp_pos[:, 1],
                                                   around_handle_reward + 0.5, around_handle_reward),
                                       around_handle_reward)
    # reward for distance of each finger from the drawer
    finger_dist_reward = torch.zeros_like(rot_reward)
    lfinger_dist = torch.abs(franka_lfinger_pos[:, 2] - cylinder_grasp_pos[:, 2])
    rfinger_dist = torch.abs(franka_rfinger_pos[:, 2] - cylinder_grasp_pos[:, 2])
    finger_dist_reward = torch.where(franka_lfinger_pos[:, 2] > cylinder_grasp_pos[:, 2],
                                     torch.where(franka_rfinger_pos[:, 2] < cylinder_grasp_pos[:, 2],
    ...
    ## [Kunal] - Adding condition to reset if arm is close to cabinet handle
